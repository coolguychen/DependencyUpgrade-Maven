...
public class MultipleModule extends SingleModule {
    ...
    public MultipleModule(String path, List<String> list) {
        projectPath = path;
        fileList = list; //传递pom文件列表
    }

    // 构造函数 传递项目路径 & 推荐类型
    public MultipleModule(String path, int _type) {
        projectPath = path;
        type = _type;
    }
...
//            System.out.println("-----------获取更高版本完毕。------------");
    ...
    public void getMultiUpgradeSolutions(String projectPath, int type) throws InterruptedException {
        // 1. 递归遍历目录
...
//            System.out.println("该路径下无pom.xml");
    ...
    public static void main(String[] args) {
        ...
        MultipleModule multi = new MultipleModule("D:\\1javawork\\multiModelDemo", 0);
        if(multi.conflictDetectBefore()) {
...
//            System.out.println("======分割线======下一个模块=======");
    ...
    public void extractCommonLibrary() {
        //1. todo 对于最后每个没有依赖冲突的结果集，提取公共依赖
        for (Map.Entry<String, List<List<Dependency>>> entry : filePath_resWithoutConflict.entrySet()) {

        }

        //2. todo 选取其中版本最高的，放到父模块中统一管理

        //3. todo 修改原来的pom文件 去掉version字段
    }
